...
class TestValidPositionObject(unittest.TestCase):
    ...
    @patch.object(redditScraperUtils.objects.validPositionObject, 'datetime', Mock(wraps=datetime.datetime))
    def test_formatDate_ShouldAddYearToDate_When_YearNotIncluded(self):
        redditScraperUtils.objects.validPositionObject.datetime.utcnow.return_value = datetime.datetime(2020, 2, 3)
        position = validPosition("MSFT", "100", "06/19")
        self.assertEqual("06/19", position.strikeDate)
